...
local function idTmpl(mfxId)
    return "PRM-_FPARTxxx-_KIT-_KF-" .. mfxId
end
...
local function mfxTypeChange(param, part, index)
    ...
    local defaultValues = get_set_mfx_default_values_sysex(Mfx_Types.SND, part, index)
    ...
    grp = {
        {
            name = "Mfx Common",
            params = {
                {type="select", id=idTmpl("SDKF_MFX_TYPE"),      name=get("MFX Type"),              default=0,   options=mfxTypes},
                {type="range",  id=idTmpl("SDKF_MFX_DRY_SEND"),  name=get("MFX Dry Send Level"),    default=127, min=get(0), max=get(127)},
                {type="range",  id=idTmpl("SDKF_MFX_CHO_SEND"),  name=get("MFX Chorus Send Level"), default=0,   min=get(0), max=get(127)},
                {type="range",  id=idTmpl("SDKF_MFX_REV_SEND"),  name=get("MFX Reverb Send Level"), default=0,   min=get(0), max=get(127)},
                {type="range",  id=idTmpl("SDKF_MFX_CTRL1_SRC"), name=get("MFX Control 1 Source"),  default=0,   min=get(0), max=get(101)},
                {type="range",  id=idTmpl("SDKF_MFX_CTRL1_SENS"),name=get("MFX Control 1 Sens"),    default=64,  min=get(1), max=get(127)},
                {type="range",  id=idTmpl("SDKF_MFX_CTRL2_SRC"), name=get("MFX Control 2 Source"),  default=0,   min=get(0), max=get(101)},
                {type="range",  id=idTmpl("SDKF_MFX_CTRL2_SENS"),name=get("MFX Control 2 Sens"),    default=64,  min=get(1), max=get(127)},
                {type="range",  id=idTmpl("SDKF_MFX_CTRL3_SRC"), name=get("MFX Control 3 Source"),  default=0,   min=get(0), max=get(101)},
                {type="range",  id=idTmpl("SDKF_MFX_CTRL3_SENS"),name=get("MFX Control 3 Sens"),    default=64,  min=get(1), max=get(127)},
                {type="range",  id=idTmpl("SDKF_MFX_CTRL4_SRC"), name=get("MFX Control 4 Source"),  default=0,   min=get(0), max=get(101)},
                {type="range",  id=idTmpl("SDKF_MFX_CTRL4_SENS"),name=get("MFX Control 4 Sens"),    default=64,  min=get(1), max=get(127)},
            }
        },
        {
            name = "Mfx",
...
function CreateMfxSndSections(main)
    for partNr = 1, 16, 1 do
        local k = "Part " .. string.format("%02d", partNr) .. " SN-D MFX"
        local mfxData = DeepCopy(mfxTemplate)
        mfxData.name = k
        mfxData.getReceiveValueSysex = function()
            return CreateReceiveMessageForBranch("PRM-_FPART" .. partNr .. "-_KIT-_KF")
        end
        main[k] = mfxData
        ...
        local function mfxChangedHandler(leafNode, response)
            ...
            if leafNode.node.id == "SDKF_MFX_TYPE" then
        ...
        for _, param in ipairs(subCommon.params) do
            local isMfxChangeType = param.id == idTmpl("SDKF_MFX_TYPE")
        ...
        for mfxNr = 0, 31, 1 do
            local id = idTmpl("SDKF_MFX_PRM" .. tostring(mfxNr + 1))
