...
local function idPt(partNr, partialNr, name)
    return "PRM-_FPART" .. partNr .. "-_PAT-_PT" .. partialNr .. "-RFPT_" .. name
end

local function makePartialGroup(partNr, partialNr)
    local function p(name) return idPt(partNr, partialNr, name) end

    local params = {
        {type="select", id=p("WAV_GID"),    name=get("Wave Group"),         default=0,  options=WaveGroup},
        {type="select", id=p("WAV_NUML"),   name=get("Wave No. L (Mono)"),  default=1,  options=WaveNames},
        {type="select", id=p("WAV_NUMR"),   name=get("Wave No. R"),         default=0,  options=WaveNames},
...
function CreatePcmsWaveSections(main)
    for partNr = 1, 16, 1 do
        local k = "Part " .. string.format("%02d", partNr) .. " PCM-S Wave"

        local grp = {}
        for partialNr = 1, 4, 1 do
            table.insert(grp, makePartialGroup(partNr, partialNr))
        end
        ...
        local sectionData = {
            ...
            getReceiveValueSysex = function()
                local msgs = {}
                for partialNr = 1, 4, 1 do
                    local branch = "PRM-_FPART"..partNr.."-_PAT-_PT"..partialNr
                    msgs = Concat(msgs, CreateReceiveMessageForBranch(branch))
                end
                return msgs
            end,
        }
