...
-- Mapping from model node ID to fake param ID suffix (for receive handler)
...
-- Per-part: currently selected note (27-88)
...
function CreateSndInstSections(main)
    for partNr = 1, 16, 1 do
        local k = "Part " .. string.format("%02d", partNr) .. " SN-D Inst"

        -- Receive handler: maps incoming SDKN_* values for the current note to fake IDs
        ...
        -- Helper: creates a setValue closure that routes to the current note's KN address
        ...
        local params = {
            -- Note selector (virtual param, no SysEx sent)
            {
                type="select",
                id=noteSelectId,
                name=get("Drum Note"),
            ...
            -- Drum instrument parameters (fake IDs, dynamic setValue)
        ...
        main[k] = {
            ...
            getReceiveValueSysex = function()
                local note = notePartMap[partNr]
                return CreateReceiveMessageForBranch("PRM-_FPART" .. partNr .. "-_KIT-_KN" .. note)
            end,
        }
    end
end
